...
class FeatureRegisterationModule:
    def __init__(self):
        super().__init__()
    ...
    def register_features(self, batch, image1, image2, strategy):
        ...
        # image1 = visibility2 * (image1 - image2_warped_onto_image1) #重点!!!!!!!!!
        # image2 = visibility1 * (image2 - image1_warped_onto_image2)
        return image2_warped_onto_image1, image1_warped_onto_image2

    def func(self, batch):
        reg_3d = [s == "3d" for s in batch["registration_strategy"]]
        image1 = batch['left_image'][reg_3d]
        image2 = batch['right_image'][reg_3d]
        right2left, left2right = self.register_features(slice_batch_given_bool_array(batch,reg_3d), image1, image2, "3d")
...
def estimate_Rt_using_camera_parameters(intrinsics1, intrinsics2, rotation1, rotation2, position1, position2):
    ...
    Rt_1_to_2, Rt_2_to_1 = geometry.get_relative_pose(  # 通过提供的相机参数,变换成相关的转换矩阵
        rotation1,
        rotation2,
        position1,
        position2,
...
def estimate_Rt_using_points(points1, points2, depth1, depth2):
    K_inv, Rt = geometry.setup_canonical_cameras(len(points1), tensor_to_infer_type_from=points1[0])  # 先虚拟构成出与相机参数相符的矩阵
    batch_points1_in_world_coordinates = []
    batch_points2_in_world_coordinates = []
    for i in range(len(points1)):
        points1_in_world_coordinates = geometry.convert_image_coordinates_to_world(# 获取现实世界坐标系下的点坐标
    ...
    Rt_1_to_2 = geometry.estimate_linear_warp(batch_points1_in_world_coordinates,
                                                      batch_points2_in_world_coordinates)  # 通过上述获得坐标点,计算出图像对变换矩阵
    Rt_2_to_1 = geometry.estimate_linear_warp(batch_points2_in_world_coordinates,
                                                      batch_points1_in_world_coordinates)
...
class DifferentiableFeatureWarper:
    def __init__(self):
        super().__init__()
    ...
    def warp(self, features_src, depth_src, src_camera_K_inv, dst_camera_K_inv,
        ...
        src_points_in_dst_camera_coords = geometry.convert_world_to_image_coordinates(
            ...
            geometry.convert_image_coordinates_to_world(  # 将图像坐标转换为dst相机坐标系下的三维坐标。
    ...
    def render_features_from_points(self, points_in_3d, features): #从三维点云数据中渲染出特征图像
        ...
        src_point_cloud = Pointclouds(#将转换后的三维点云和特征数据封装成点云对象
            points=geometry.convert_to_pytorch3d_coordinate_system(points_in_3d),#将输入的三维点云坐标转换为 PyTorch3D 坐标系
