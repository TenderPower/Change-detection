...
from .utils_ import get_warp_flow, upsample2d_flow_as, get_grid
...
class HomoNet(nn.Module):
    ...
    def forward(self, data_batch):
        ...
        H_flow_f = (self.basis.to(forward_fea.device) * weight_f).sum(1).reshape(bs, 2, h_patch, w_patch) #_f看成图片2->1

        # ========================backward===================================
        backward_fea = torch.cat([img2_patch_fea, img1_patch_fea], dim=1)
        weight_b = self.h_net(backward_fea)
        H_flow_b = (self.basis.to(backward_fea.device) * weight_b).sum(1).reshape(bs, 2, h_patch, w_patch) #_b看成图片1->2
        ...
        img1_patch_warp_fea, img2_patch_warp_fea = list(map(self.fea_extra, [warp_img1_patch, warp_img2_patch]))
        # ============================= mask==========================================
